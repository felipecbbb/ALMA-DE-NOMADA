...
export async function POST(req: Request) {
  try {
    const { name, email, phone, message, newsletter } = await req.json();
    ...
    await sendEmail({
      ...
      html: contactFormHtml({ name, email, phone: phone ?? "", message, newsletter: !!newsletter }),
      text: contactFormText({ name, email, phone: phone ?? "", message, newsletter: !!newsletter }),
    });
